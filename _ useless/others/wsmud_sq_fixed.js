...
// - - - - - 苏轻添加内容 - - - - -
WG.add_hook(["dialog", "status", "msg", "text"], function(data) {
    // 控制台记录
    // console.dir(data);
    ...
    if (data.type == "dialog") {
        if (data.dialog == "score") {
            userId = data.id;
            WG.Send("tm 你的ID: " + userId);
        } else if (data.dialog == "party") {
            WG.Send("tm 已获取帮派数据");
            partyData.name = data.name;
            partyData.level = data.level;
            partyData.roles = data.roles;

            // var string = data.level + "级帮派" + data.name;
            var log = {};
            var hy = 0;
            var string = "";

            for (var i = 0; i < partyData.roles.length; i++) {
                var nameArray = partyData.roles[i].name.split(" ");

                hy = hy + parseInt(partyData.roles[i].sc);
                if (partyData.roles[i].sc < 40) string = string + partyData.roles[i].name + " ";
            }
            var message = "tm 统计"
                      + "&#010;【帮派】今日已获得帮派活跃: " + hy
                      + "&#010;【帮派】今日还可以获得活跃: " + ((partyData.level+1)*400-hy)
                      + "&#010;【帮派】未完成任务的成员有: " + string;
            sendMessage(message);
        }
    }
    ...
    if (data.type == "status") {
        ...
        if (data.id == userId) {
            ...
            if (data.action == "add") WG.Send("tm 自身状态增加 " + data.name + " " + data.sid);

            if (data.action == "remove") {
                WG.Send("tm 自身状态消失 " + data.sid);
                //太上忘情
                if (data.sid == "mingyu") WG.Send("perform force.wang;");
                //明玉 && 紫气东来
                else if (data.sid == "force") WG.Send("perform force.power;perform force.xi;");
                //踏歌行
                else if (data.sid == "dodge") WG.Send("perform dodge.power");
                //无招
                else if (data.sid == "weapon") WG.Send("perform sword.wu");
            }

            if (data.action == "refresh") {
                WG.Send("tm 自身状态刷新 " + data.sid + " count:" + data.count);
                // 唱仙法
                if (data.sid == "parry" && data.count < 10) WG.Send("perform parry.chang");
            }

        // 别人
        } else {
            if (data.action == "remove") {
                if (data.sid == "yihua") WG.Send("perform parry.yi;tm timeOver: <敌>移花");
                else if (data.sid == "faint") WG.Send("perform unarmed.duo;tm timeOver: <敌>昏迷");
                else WG.Send("tm removeBuff: " + data.sid);
            }
        }
    ...
    // - - - - - 监控所有的频道 - - - - -
        ...
        // 帮派频道
            ...
            // 如果消息内容是数字 0 ~ 5
            if (data.content == "0" || data.content == "1" || data.content == "2"
             || data.content == "3" || data.content == "4" || data.content == "5") {
                dpArray[parseInt(data.content)].push(data.name); // 保存到数组
            }
            if (data.content.indexOf("<hio>") >=0 && data.content.indexOf("</hio>") >=0) {
                WG.Send("pty 出！橙！装！");
            }
        } else if (data.name == "金古易") { // 金古易
                WG.Send("chat 恭喜！");
        }
    // - - - - - 监控当前房间字幕 - - - - -
    } else if (data.type == "text") {
        if (data.msg.indexOf("<hio>帝魄碎片</hio>") >= 0) {
            WG.Send("tm 获得了一块帝魄碎片");
        }
    }
});
...
function sendMessage(message) {
    $("span[WG='WG']").attr("cmd", message).click();
}
//@end
})();
